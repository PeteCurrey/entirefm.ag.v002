    ...
    <main className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white mt-1">The FM Briefing Dashboard</h1>
      ...
      {!domainAuth.canSend && (
        <div className="bg-amber-950/40 border border-amber-800/60 rounded-xl p-4 flex items-start gap-3 text-amber-200">
          ...
          <div className="text-xs space-y-1">
            <h4 className="font-bold text-amber-300">EMAIL DELIVERY ADAPTER: OFFLINE MOCK MODE</h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          <div className="mt-2 text-2xl font-bold text-white font-mono">{activeSubscribers}</div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          <div className="mt-2 text-2xl font-bold text-white font-mono">{draftCampaigns.length}</div>
          <div className="mt-1 text-[11px] text-zinc-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
                ...
                {draftCampaigns.map((camp) => (
                  <div key={camp.id} className="py-4 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <h4 className="text-sm font-bold text-white mt-1.5">{camp.name}</h4>
                      <p className="text-xs text-zinc-400 mt-0.5 line-clamp-1">{camp.subject}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Link
                        href={`/admin/newsletter/${camp.id}`}
                        className="text-xs bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1.5 rounded-lg font-semibold border border-zinc-700"
                      >
                        Edit / Preview
                      </Link>
                    </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            >
              <h4 className="text-xs font-bold text-white group-hover:text-pink-300 flex items-center gap-1.5">
            ...
            >
              <h4 className="text-xs font-bold text-white group-hover:text-blue-300 flex items-center gap-1.5">
            ...
            >
              <h4 className="text-xs font-bold text-white group-hover:text-red-300 flex items-center gap-1.5">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-4">
            ...
            <form action="/api/admin/newsletter/automation" method="POST">
              <button
                type="submit"
                className="w-full bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold py-2.5 px-4 rounded-lg border border-zinc-700 flex items-center justify-center gap-2"
              >
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-3">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between py-1">
                ...
                <span className="text-white font-mono font-bold">Optional</span>
              </div>
            </div>
          </div>
        </div>
