      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {products.map((p) => (
          <div key={p.id} className="bg-white border border-slate-200 p-6 rounded-sm shadow-sm space-y-3">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-light">{p.internal_id}</span>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-baseline justify-between">
              <span className="text-xl font-light text-slate-900">£{p.price_gbp.toLocaleString()}</span>
              <span className="text-[10.5px] font-normal text-slate-500">+ VAT / {p.billing_frequency.toLowerCase()}</span>
            </div>
          </div>
        ))}
      </div>
