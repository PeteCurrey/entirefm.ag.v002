      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {products.map((p) => (
          <div key={p.id} className="bg-white border border-slate-200 p-6 rounded-sm shadow-sm space-y-3">
            <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400 font-bold">{p.internal_id}</span>
            <h3 className="text-base font-bold text-slate-900">{p.public_name}</h3>
            <p className="text-xs text-slate-600 font-light">{p.description}</p>
            <div className="pt-3 border-t border-slate-100 flex items-baseline justify-between">
              <span className="text-xl font-mono font-bold text-slate-900">£{p.price_gbp.toLocaleString()}</span>
