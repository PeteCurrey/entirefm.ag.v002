    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-sm shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <Link href="/admin/suppliers/commercial" className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-mono font-bold rounded transition-colors">
            Commercial Hub
          </Link>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-sm p-6 shadow-sm">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 mb-4">
              ...
              <Link href="/admin/suppliers/landscape" className="text-xs font-mono text-brand-pink font-semibold underline">
                Open Full Matrix
              </Link>
            </div>
                ...
                {supplierApplications.slice(0, 4).map((app) => (
                  <div key={app.id} className="py-3 space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-900">{app.company || app.name}</span>
                    ...
                    <Link
                      ...
                      className="text-[11px] font-mono text-brand-pink font-semibold hover:underline inline-block pt-1"
                    >
                      Review &amp; Convert &rarr;
                    </Link>
                  </div>
            ...
            <div className="divide-y divide-slate-100">
              {targets.slice(0, 3).map((t) => (
                <div key={t.id} className="py-3 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-900">{t.company_name}</span>
