    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/60 p-4">
          <div className="font-medium text-[10px] uppercase text-brand-mist/50">Active SLA Radar</div>
          <div className="mt-1 text-2xl font-light text-white">{slaRisks.length}</div>
        </div>
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/60 p-4">
          <div className="font-medium text-[10px] uppercase text-brand-mist/50">At Risk (&lt;60m)</div>
          <div className="mt-1 text-2xl font-light text-amber-300">0</div>
        </div>
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/60 p-4">
          <div className="font-medium text-[10px] uppercase text-brand-mist/50">Breached Today</div>
          <div className="mt-1 text-2xl font-light text-rose-400">0</div>
        </div>
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/60 p-4">
          <div className="font-medium text-[10px] uppercase text-brand-mist/50">On-Track Compliance</div>
      ...
      {slaRisks.length > 0 ? (
        <div className="overflow-x-auto rounded-lg border border-brand-edge-dark bg-brand-carbon/40 shadow-sm">
          <table className="w-full min-w-[60rem] border-collapse text-left text-[12.5px]">
            <thead>
              <tr className="border-b border-brand-edge-dark bg-brand-void/50 font-medium text-[10.5px] uppercase tracking-wider text-brand-mist/50">
              ...
              {slaRisks.map((wo) => (
                <tr key={wo.id} className="text-brand-mist/80 hover:bg-brand-void/40">
                  <td className="px-5 py-4">
                    <div className="font-normal text-[11px] text-white">{wo.work_order_number}</div>
                  ...
                  <td className="px-5 py-4">
                    <span className="rounded bg-brand-edge-dark px-2 py-0.5 font-normal text-[10px] text-brand-mist/80">
                      {wo.priority}
                    </span>
                  </td>
