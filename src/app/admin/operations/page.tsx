    ...
    <main className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          ...
          <div className="mt-1 text-xl font-light text-red-400 font-mono">{metrics.criticalExceptionsCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">SLA AT RISK</div>
          <div className="mt-1 text-xl font-light text-amber-400 font-mono">{metrics.slaAtRiskCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">OVERDUE PPM</div>
          <div className="mt-1 text-xl font-light text-purple-400 font-mono">{metrics.overduePpmCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">CRITICAL DEFECTS</div>
          <div className="mt-1 text-xl font-light text-pink-400 font-mono">{metrics.openCriticalDefectsCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">CLIENT BOTTLENECKS</div>
          <div className="mt-1 text-xl font-light text-blue-400 font-mono">{metrics.clientActionsOverdueCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">SUPPLY GAPS</div>
          <div className="mt-1 text-xl font-light text-zinc-300 font-mono">{metrics.contractorCoverageGapsCount}</div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-zinc-800">
              {exceptions.map((exc) => (
                <tr key={exc.id} className="hover:bg-zinc-800/40">
                  ...
                  <td className="py-3.5 px-4 font-mono text-emerald-400 font-light">{exc.status}</td>
