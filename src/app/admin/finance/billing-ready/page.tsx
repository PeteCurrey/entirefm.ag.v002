    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-brand-carbon border border-brand-edge-dark p-4 rounded-xl font-mono">
          ...
          <div className="text-2xl font-bold text-emerald-400 mt-1">{queue.length} items</div>
        ...
        <div className="bg-brand-carbon border border-brand-edge-dark p-4 rounded-xl font-mono">
          ...
          <div className="text-2xl font-bold text-amber-400 mt-1">{leakage.length} jobs</div>
            ...
            <tbody className="divide-y divide-brand-edge-dark/60">
              {queue.map(item => (
                <tr key={item.id} className="hover:bg-brand-edge-dark/20 transition-colors">
                  ...
                  <td className="p-3.5 text-white font-semibold">£{(Number(item.billable_net_gbp || item.net_revenue_gbp) || 0).toFixed(2)}</td>
                  <td className="p-3.5 text-brand-mist/80">£{(Number(item.billable_tax_gbp) || 0).toFixed(2)}</td>
                  <td className="p-3.5 font-bold text-brand-electric">£{(Number(item.billable_gross_gbp || item.gross_revenue_gbp) || 0).toFixed(2)}</td>
