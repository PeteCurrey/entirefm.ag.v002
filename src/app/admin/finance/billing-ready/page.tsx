  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-[#E8E8E5] p-4 rounded-xl font-normal">
          <div className="text-[11px] uppercase text-[#6D6D68]">Ready to Invoice</div>
          <div className="text-2xl font-extralight text-emerald-400 mt-1">{queue.length} items</div>
          <div className="text-xs text-[#6D6D68] mt-0.5">£{totalBillable.toFixed(2)} total value</div>
        </div>
        <div className="bg-white border border-[#E8E8E5] p-4 rounded-xl font-normal">
          <div className="text-[11px] uppercase text-[#6D6D68]">Unbilled Completed Work</div>
          <div className="text-2xl font-extralight text-amber-400 mt-1">{leakage.length} jobs</div>
          <div className="text-xs text-[#6D6D68] mt-0.5">Requires billing item creation</div>
        </div>
        <div className="bg-white border border-[#E8E8E5] p-4 rounded-xl font-normal">
          <div className="text-[11px] uppercase text-[#6D6D68]">Aged Unbilled (&gt;30d)</div>
          <div className="text-2xl font-extralight text-red-400 mt-1">
            {leakage.filter(l => l.ageingDays > 30).length} jobs
          </div>
          <div className="text-xs text-[#6D6D68] mt-0.5">High margin leakage risk</div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-[#E8E8E5]">
              {queue.map(item => (
                <tr key={item.id} className="hover:bg-[#F5F5F3] transition-colors">
                  ...
                  <td className="p-3.5">
                    <span className="px-2 py-0.5 rounded text-[10px] bg-[#F5F5F3] text-[#111111]">
                  ...
                  <td className="p-3.5 text-[#6D6D68]">£{(Number(item.billable_tax_gbp) || 0).toFixed(2)}</td>
