...
const FREQ_COLOURS: Record<string, string> = {
  ...
  BIENNIAL: 'bg-[#F5F5F3] text-[#6D6D68]',
  FIVE_YEARLY: 'bg-[#F5F5F3] text-[#6D6D68]',
              ...
              <tbody className="divide-y divide-[#E8E8E5]">
                {sources.map((s) => (
                  <tr key={s.id} className="text-[#6D6D68] hover:bg-[#FAFAF8]">
                    ...
                    <td className="px-5 py-4 text-[#6D6D68]">{s.provider}</td>
                    <td className="px-5 py-4 font-normal text-[10px] text-[#9A9A95]">{s.source_type}</td>
                    <td className="px-5 py-4 font-normal text-[11px] text-[#6D6D68]">{s.version}</td>
                    <td className="px-5 py-4">
                      <span className={`rounded px-2 py-0.5 font-normal text-[10px]${LICENSING_COLOURS[s.licensing_status] ?? 'bg-[#F5F5F3] text-[#6D6D68]'}`}>
              ...
              <tbody className="divide-y divide-[#E8E8E5]">
                {requirements.map((r) => (
                  <tr key={r.id} className="text-[#6D6D68] hover:bg-[#FAFAF8]">
                    ...
                    <td className="px-5 py-4 font-normal text-[11px] text-[#6D6D68]">{r.asset_class}</td>
                    <td className="px-5 py-4 font-light text-white">{r.title}</td>
                    <td className="px-5 py-4">
                      <span className={`rounded px-2 py-0.5 font-normal text-[10px]${FREQ_COLOURS[r.frequency] ?? 'bg-[#F5F5F3] text-[#6D6D68]'}`}>
                        {r.frequency.replace(/_/g, ' ')}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-[#6D6D68]">{r.required_trade}</td>
                    <td className="px-4 py-4 text-right font-normal text-[11px] text-[#6D6D68]">{r.expected_duration_hours}h</td>
                    <td className="px-5 py-4">
                      {r.statutory_relevance ? (
                        ...
                        <span className="text-[#9A9A95] text-[11px]">—</span>
                      )}
                    </td>
                  </tr>
                ))}
