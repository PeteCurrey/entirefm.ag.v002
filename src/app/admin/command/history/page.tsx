    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="rounded-lg border border-red-900/40 bg-red-950/20 p-8 max-w-md text-center">
          ...
          <div className="text-[#111111] font-light">enterprise_intelligence:history_view permission required.</div>
        </div>
      </div>
    );
  }
      ...
      {querySessions.length === 0 ? (
        ...
        <div className="space-y-4">
          {querySessions.map((s: any) => (
            <div key={s.id} className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="text-[11px] font-normal text-[#9A9A95]">
              ...
              <div className="space-y-2">
                {(s.messages || []).slice(0, 6).map((m: any) => (
                  <div key={m.id} className={`flex gap-3 text-[12px] ${m.role === 'USER' ? '' : 'pl-4'}`}>
                    ...
                    <span className="text-[#6D6D68] line-clamp-2">{m.content}</span>
                    {m.data_status && m.data_status !== 'LIVE' && (
                      <span className="ml-auto shrink-0 text-[9px] font-normal text-[#9A9A95] self-center">
                        {m.data_status}
                      </span>
                    )}
                  </div>
