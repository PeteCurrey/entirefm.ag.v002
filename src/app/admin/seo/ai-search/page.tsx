    ...
    <main className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-medium">TRACKED QUESTIONS</div>
          <div className="mt-1 text-xl font-light text-white">{TRACKED_AI_QUESTIONS.length}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-medium">PENDING MANUAL CHECK</div>
          <div className="mt-1 text-xl font-light text-amber-400">{pendingCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-medium">CITATIONS OBSERVED</div>
          <div className="mt-1 text-xl font-light text-zinc-400">{testedCount} (No fabricated stats)</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-medium">SOURCE RIGOUR</div>
          <div className="mt-1 text-xl font-light text-purple-400">100% Verified Standards</div>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-zinc-800 flex items-center justify-between">
          ...
          <span className="text-xs text-amber-400 font-normal">Observation Protocol: External Manual Checks Required</span>
        </div>
            ...
            {TRACKED_AI_QUESTIONS.map((t) => (
              <tr key={t.id} className="hover:bg-zinc-800/40">
                <td className="py-3.5 px-4 font-normal text-white max-w-sm">
                  <div>{t.question}</div>
                  <div className="text-[11px] font-normal text-zinc-500 mt-0.5">{t.notes}</div>
                </td>
                <td className="py-3.5 px-4">
                  <Link href={t.primaryUrl} className="text-emerald-400 hover:underline font-normal">
                    {t.primaryUrl}
                  </Link>
                </td>
                <td className="py-3.5 px-4">
                  <span className="text-[10px] uppercase px-2 py-0.5 rounded font-light border bg-amber-950/60 text-amber-300 border-amber-800/40">
                    {t.status}
                  </span>
                </td>
                <td className="py-3.5 px-4 font-normal text-zinc-300">{t.engineTarget}</td>
                <td className="py-3.5 px-4 font-normal text-zinc-400">{t.supportingSource}</td>
              </tr>
            ))}
          </tbody>
        </table>
