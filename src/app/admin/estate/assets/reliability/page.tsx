    ...
    <div className="space-y-8">
      ...
      {signals.length > 0 && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-lg border border-[#E8E8E5] bg-brand-surface p-4">
            <p className="text-[11px] uppercase tracking-wider text-[#9A9A95]">Active Signals</p>
            <p className="mt-1 text-2xl font-semibold text-[#111111]">{signals.length}</p>
          </div>
          <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-4">
            <p className="text-[11px] uppercase tracking-wider text-[#9A9A95]">Critical</p>
            <p className="mt-1 text-2xl font-semibold text-red-400">{criticalCount}</p>
          </div>
          <div className="rounded-lg border border-orange-500/20 bg-orange-500/5 p-4">
            <p className="text-[11px] uppercase tracking-wider text-[#9A9A95]">High</p>
            <p className="mt-1 text-2xl font-semibold text-orange-400">{highCount}</p>
          </div>
          <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-4">
            <p className="text-[11px] uppercase tracking-wider text-[#9A9A95]">Warning</p>
      ...
      {signals.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-[#6D6D68]">Active Signals — most recent first</h3>
          {signals.map((signal) => (
            <div
              key={signal.id}
              className={`rounded-lg border px-4 py-4 ${SEVERITY_STYLES[signal.severity] ?? 'border-[#E8E8E5] bg-brand-surface text-[#111111]'}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-sm font-medium text-[#111111]">{signal.title}</p>
                  <p className="text-xs text-[#6D6D68] leading-relaxed">{signal.description}</p>
                  <div className="flex flex-wrap gap-3 pt-1 text-[10px] text-[#9A9A95]">
                    <span>Asset: <span className="text-[#6D6D68]">{signal.asset_id.slice(0, 8)}…</span></span>
                    <span>Criticality: <span className="text-[#6D6D68]">{signal.asset_context_snapshot?.criticality ?? '—'}</span></span>
                    <span>Condition: <span className="text-[#6D6D68]">{signal.asset_context_snapshot?.condition ?? '—'}</span></span>
                    <span>Failures (90d): <span className="text-[#6D6D68]">{signal.asset_context_snapshot?.failure_count_90d ?? '—'}</span></span>
                    <span>Policy: <span className="text-[#6D6D68]">v{signal.policy_version}</span></span>
                    <span>Generated: <span className="text-[#6D6D68]">{new Date(signal.generated_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span></span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="rounded-lg border border-[#E8E8E5] bg-brand-surface px-4 py-4">
        <h3 className="text-sm font-medium text-[#111111] mb-2">Determinism Guarantee</h3>
