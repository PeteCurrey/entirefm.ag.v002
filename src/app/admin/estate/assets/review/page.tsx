  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex gap-4">
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm px-5 py-4 text-center">
          <div className="text-2xl font-extralight text-[#111111]">{candidates.length}</div>
          <div className="mt-1 text-[11px] text-[#9A9A95] uppercase tracking-wider">Candidates Pending</div>
        </div>
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm px-5 py-4 text-center">
          <div className="text-2xl font-extralight text-[#111111]">{duplicates.length}</div>
          <div className="mt-1 text-[11px] text-[#9A9A95] uppercase tracking-wider">Duplicates Pending</div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-[#E8E8E5]">
                {candidates.map((c) => (
                  <tr key={c.id} className="text-[#6D6D68] hover:bg-[#FAFAF8]">
                    ...
                    <td className="px-5 py-4 text-[#6D6D68]">{c.proposed_category || '—'}</td>
                    <td className="px-5 py-4 text-[#6D6D68]">{c.proposed_manufacturer || '—'}</td>
                    <td className="px-5 py-4">
                      <span className={`rounded px-2 py-0.5 font-normal text-[10px]${SOURCE_COLOURS[c.source_type] ?? 'bg-[#F5F5F3] text-[#6D6D68]'}`}>
        ...
        {duplicates.length > 0 ? (
          <div className="overflow-x-auto rounded-lg border border-[#E8E8E5] bg-white shadow-sm">
            <table className="w-full border-collapse text-left text-[12.5px]">
              ...
              <tbody className="divide-y divide-[#E8E8E5]">
                {duplicates.map((d) => (
                  <tr key={d.id} className="text-[#6D6D68] hover:bg-[#FAFAF8]">
                    <td className="px-5 py-4 font-normal text-[11px] text-[#6D6D68]">
                      {Math.round(d.confidence_score * 100)}%
                    </td>
                    <td className="px-5 py-4 text-[11.5px] text-[#6D6D68]">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-3">
                        ...
                        <button className="text-[11px] text-[#9A9A95] hover:text-[#111111]">Keep Separate</button>
                      </div>
                    </td>
                  </tr>
                ))}
