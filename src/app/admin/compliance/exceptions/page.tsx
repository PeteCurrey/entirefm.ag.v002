        ...
        <div className="overflow-x-auto rounded-lg border border-[#E8E8E5] bg-white shadow-sm">
          <table className="w-full min-w-[64rem] border-collapse text-left text-[12.5px]">
            ...
            <tbody className="divide-y divide-[#E8E8E5]">
              {exceptions.map((exc) => (
                <tr key={exc.id} className="text-[#6D6D68] hover:bg-[#FAFAF8]">
                  <td className="px-5 py-4">
                    <div className="font-light text-[#111111]">{exc.site?.name || 'Site'}</div>
                    <div className="text-[11.5px] text-[#9A9A95]">
                      {exc.asset?.name || 'System Level Exception'}
                    </div>
                  </td>
                  <td className="px-5 py-4 max-w-xs">
                    <div className="font-normal text-[#111111]">{exc.exception_type}</div>
                    <div className="text-[11.5px] text-[#6D6D68] line-clamp-2">{exc.reason}</div>
                  </td>
                  <td className="px-5 py-4">
                    <span
