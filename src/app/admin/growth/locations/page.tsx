    ...
    <main className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-extralight text-white mt-1">Location Commercial Performance</h1>
          ...
          <tbody className="divide-y divide-zinc-800">
            {clusters.map((c) => (
              <tr key={c.key} className="hover:bg-zinc-800/40">
                ...
                <td className="py-3.5 px-4 text-pink-400 font-light">{c.leadsCount}</td>
                <td className="py-3.5 px-4 text-emerald-400 font-light">{c.qualifiedLeadsCount}</td>
                <td className="py-3.5 px-4 font-normal text-zinc-400">{c.conversionRatePct}%</td>
              </tr>
            ))}
          </tbody>
        </table>
