    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-4">
          <div className="font-medium text-[11px] uppercase tracking-wider text-[#9A9A95]">Sell Total (Net)</div>
          <div className="mt-1 text-[20px] font-normal text-[#111111]">
        ...
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-4">
          <div className="font-medium text-[11px] uppercase tracking-wider text-[#9A9A95]">Expected Cost (Internal)</div>
          <div className="mt-1 text-[20px] font-normal text-[#6D6D68]">
        ...
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-4">
          <div className="font-medium text-[11px] uppercase tracking-wider text-[#9A9A95]">Projected Margin</div>
        ...
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-4">
          <div className="font-medium text-[11px] uppercase tracking-wider text-[#9A9A95]">Client PO Status</div>
          <div className="mt-1 text-[16px] font-normal text-[#111111]">
            {quote.client_po_ref ? (
              ...
              <span className="text-[#9A9A95]">Not Required</span>
            )}
          </div>
          <div className="mt-1 font-normal text-[11px] text-[#9A9A95]">
      ...
      <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-5">
        <h3 className="font-medium text-[12px] uppercase tracking-wider text-[#6D6D68]">Commercial Scope</h3>
        <p className="mt-2 text-[13px] text-[#6D6D68] leading-relaxed">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-[12px] uppercase tracking-wider text-[#6D6D68]">Quoted Line Items</h3>
          <span className="font-normal text-[11px] text-[#9A9A95]">{lines?.length || 0} line(s)</span>
        </div>
          ...
          <table className="w-full min-w-[55rem] border-collapse text-left text-[12.5px]">
            ...
            <tbody className="divide-y divide-[#E8E8E5]">
              {lines && lines.length > 0 ? (
                lines.map((l, idx) => (
                  <tr key={l.id || idx} className="text-[#6D6D68] hover:bg-[#FAFAF8]">
                    <td className="px-5 py-4">
                      <span className="rounded bg-[#F5F5F3] px-2 py-0.5 font-normal text-[10px] text-brand-electric-bright">
                        {l.line_type}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="px-5 py-6 text-center text-[#9A9A95]">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-5">
          <h3 className="font-medium text-[12px] uppercase tracking-wider text-[#6D6D68]">Quote Version Snapshots</h3>
          <div className="mt-4 space-y-3">
            {versions && versions.length > 0 ? (
              versions.map((v) => (
                <div key={v.id} className="flex items-center justify-between rounded border border-[#E8E8E5] bg-[#FAFAF8] p-3 text-[12px]">
                  <div>
                    <span className="font-light text-[#111111]">Version {v.version}</span>
                    <p className="text-[11px] text-[#6D6D68] mt-0.5">{v.change_reason}</p>
                  </div>
              ...
              <p className="text-[12px] text-[#9A9A95]">Initial version (v1) active.</p>
            )}
          </div>
        </div>

        <div className="rounded-lg border border-[#E8E8E5] bg-white shadow-sm p-5">
          <h3 className="font-medium text-[12px] uppercase tracking-wider text-[#6D6D68]">AI Governance & Source Provenance</h3>
          <div className="mt-4 space-y-3">
            {provenance && provenance.length > 0 ? (
              provenance.map((p, idx) => (
                <div key={p.id || idx} className="rounded border border-[#E8E8E5] bg-[#FAFAF8] p-3 text-[12px]">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-[11.5px] text-[#6D6D68] mt-1">{p.pricing_rule_applied || 'Standard deterministic rule calculation'}</p>
                </div>
              ))
            ) : (
              <p className="text-[12px] text-[#9A9A95]">No external source provenance records found.</p>
            )}
          </div>
        </div>
      </div>
