    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-4">
          <div className="font-mono text-[11px] uppercase tracking-wider text-brand-mist/50">Sell Total (Net)</div>
          <div className="mt-1 font-mono text-[20px] font-normal text-white">
        ...
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-4">
          <div className="font-mono text-[11px] uppercase tracking-wider text-brand-mist/50">Expected Cost (Internal)</div>
          <div className="mt-1 font-mono text-[20px] font-normal text-brand-mist/90">
        ...
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-4">
          <div className="font-mono text-[11px] uppercase tracking-wider text-brand-mist/50">Projected Margin</div>
          <div className="mt-1 font-mono text-[20px] font-normal text-emerald-400">
        ...
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-4">
          <div className="font-mono text-[11px] uppercase tracking-wider text-brand-mist/50">Client PO Status</div>
          <div className="mt-1 font-mono text-[16px] font-normal text-white">
            {quote.client_po_ref ? (
      ...
      <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-5">
        <h3 className="font-mono text-[12px] uppercase tracking-wider text-brand-mist/60">Commercial Scope</h3>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-mono text-[12px] uppercase tracking-wider text-brand-mist/60">Quoted Line Items</h3>
          <span className="font-mono text-[11px] text-brand-mist/40">{lines?.length || 0} line(s)</span>
        </div>
                ...
                lines.map((l, idx) => (
                  <tr key={l.id || idx} className="text-brand-mist/80 hover:bg-brand-void/40">
                    <td className="px-5 py-4">
                      <span className="rounded bg-brand-edge-dark px-2 py-0.5 font-mono text-[10px] text-brand-electric-bright">
                        {l.line_type}
                      </span>
                    </td>
                    <td className="px-5 py-4 font-mono text-[12px] text-white">
                      {l.description}
                    </td>
                    <td className="px-5 py-4 font-mono text-[11.5px] text-brand-mist/60">
                      {l.quantity}
                    </td>
                    <td className="px-5 py-4 font-mono text-[12px] text-brand-mist/80">
                      £{Number(l.unit_price_gbp).toFixed(2)}
                    </td>
                    <td className="px-5 py-4 font-mono text-[12.5px] font-normal text-white">
                      £{Number(l.total_gbp).toFixed(2)}
                    </td>
                    <td className="px-5 py-4 text-right font-mono text-[11px] text-brand-mist/50">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-5">
          <h3 className="font-mono text-[12px] uppercase tracking-wider text-brand-mist/60">Quote Version Snapshots</h3>
            ...
            {versions && versions.length > 0 ? (
              versions.map((v) => (
                <div key={v.id} className="flex items-center justify-between rounded border border-brand-edge-dark/60 bg-brand-void/40 p-3 text-[12px]">
                  <div>
                    <span className="font-mono font-light text-white">Version {v.version}</span>
        ...
        <div className="rounded-lg border border-brand-edge-dark bg-brand-carbon/40 p-5">
          <h3 className="font-mono text-[12px] uppercase tracking-wider text-brand-mist/60">AI Governance & Source Provenance</h3>
          <div className="mt-4 space-y-3">
            {provenance && provenance.length > 0 ? (
              provenance.map((p, idx) => (
                <div key={p.id || idx} className="rounded border border-brand-edge-dark/60 bg-brand-void/40 p-3 text-[12px]">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[11px] text-brand-electric-bright">{p.source_type}</span>
                    <span className="font-mono text-[10px] text-emerald-400">Confidence: {(Number(p.ai_confidence_score || 0.85) * 100).toFixed(0)}%</span>
                  </div>
