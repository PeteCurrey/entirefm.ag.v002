    ...
    <main className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white mt-1">Commercial Control Centre</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          ...
          <div className="mt-1 text-xl font-bold text-white font-mono">{metrics.qualifiedOpportunitiesCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">PROPOSALS OUT</div>
          <div className="mt-1 text-xl font-bold text-pink-400 font-mono">{metrics.proposalsOutCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">SURVEYS PENDING</div>
          <div className="mt-1 text-xl font-bold text-blue-400 font-mono">{metrics.surveysPendingCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">OVERDUE TASKS</div>
          <div className="mt-1 text-xl font-bold text-amber-400 font-mono">{metrics.overdueTasksCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">WON THIS MONTH</div>
          <div className="mt-1 text-xl font-bold text-emerald-400 font-mono">{metrics.wonThisMonthCount}</div>
        </div>

        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-[10px] uppercase font-mono">PIPELINE VALUE</div>
          <div className="mt-1 text-xl font-bold text-purple-400 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
                ...
                {tasks.slice(0, 6).map((task) => {
                  ...
                  return (
                    <div key={task.id} className="py-3 flex items-center justify-between gap-3 text-xs">
                      ...
                      {isOverdue ? (
                        <span className="text-[10px] font-mono text-red-400 font-bold whitespace-nowrap">
                          OVERDUE
                        </span>
                      ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            >
              <h4 className="text-xs font-bold text-white group-hover:text-pink-300 flex items-center gap-1">
            ...
            >
              <h4 className="text-xs font-bold text-white group-hover:text-blue-300 flex items-center gap-1">
            ...
            >
              <h4 className="text-xs font-bold text-white group-hover:text-emerald-300 flex items-center gap-1">
              ...
              <div className="divide-y divide-zinc-800">
                {opportunities.slice(0, 5).map((opp) => (
                  <div key={opp.id} className="py-3 flex items-center justify-between text-xs">
                    <div>
                      <div className="font-bold text-white">{opp.company}</div>
                      <div className="text-[11px] text-zinc-400">
                        {opp.service} · {opp.location}
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded bg-zinc-800 text-pink-400 font-semibold">
                        {opp.stage}
                      </span>
