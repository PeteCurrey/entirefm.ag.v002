      ...
      <div className="my-6 rounded-2xl border border-indigo-200 bg-indigo-50/70 p-6 sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-normal text-white shadow-xs transition-colors hover:bg-slate-800 shrink-0"
          >
          ...
          <div className="text-slate-700 leading-relaxed space-y-4">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {COOKIE_INVENTORY.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="px-3 py-2.5 text-[11px] font-normal text-slate-600">
                          {item.statutoryBasis}
                        </td>
                      </tr>
                    ))}
            ...
            {section.id === 'managing-preferences' && (
              <div className="pt-2">
                <button
                  onClick={openCookiePreferences}
                  className="inline-flex items-center gap-2 rounded-lg border border-indigo-600 bg-white px-4 py-2 text-xs font-normal text-indigo-700 hover:bg-indigo-50"
                >
