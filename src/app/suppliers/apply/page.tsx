...
export default function ApplyPage() {
  ...
  const whatHappensNext = [
    ...
    { step: '05', title: 'Review & Submit', desc: 'Verify all details and complete applicant statements.' },
    { step: '06', title: 'Technical Desk Review', desc: 'Technical due diligence against our Assurance Framework.' },
    { step: '07', title: 'Banking Dual-Control', desc: 'Voice-verified bank remittance details for safe BACS payments.' },
    { step: '08', title: 'Scoped Portal Activation', desc: 'Activated on EntireCAFM and eligible for live job dispatch.' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 flex flex-col font-sans">
      ...
      <main id="main" className="flex-grow">
        ...
        {/* 2. CANONICAL APPLICATION ENTRY SECTION */}
        <section id="application-start" className="py-20 sm:py-24 bg-white border-b border-slate-200 scroll-mt-12">
          <div id="application-form" className="container-custom max-w-4xl space-y-12">
            ...
            {/* Primary Action Card */}
            <div className="bg-[#FAF9FB] border border-slate-300 p-8 sm:p-12 rounded-sm shadow-sm space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-600 font-light leading-relaxed max-w-2xl">
                  Create your supplier profile, declare your engineering disciplines, and complete the information required for EntireFM supplier assurance.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-2">
                <Link
                  ...
                  className="btn-primary text-xs py-3.5 px-8 text-center flex-1 flex items-center justify-center gap-2 shadow-sm hover:shadow"
                >
                ...
                <Link
                  ...
                  className="btn-secondary text-xs py-3.5 px-6 text-center flex-1 flex items-center justify-center gap-2"
                >
              ...
              <div className="pt-4 flex items-start gap-2.5 text-xs text-slate-500 font-light border-t border-slate-200/80">
                <Clock className="h-4 w-4 text-slate-400 shrink-0 mt-0.5" />
                <span>
                  <strong className="text-slate-700 font-medium">Save &amp; Return:</strong> You do not need to complete everything in one sitting. Your progress is saved as you complete each section and can be resumed at any time from the Supplier Portal.
                </span>
              </div>
            </div>

            {/* Before You Start Checklist */}
            <div className="bg-white border border-slate-200 p-8 sm:p-10 rounded-sm shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
                {whatHappensNext.map((item) => (
                  <div key={item.step} className="p-5 bg-[#FAF9FB] border border-slate-200 rounded-sm space-y-2 flex flex-col justify-between">
                    <div>
                      <span className="text-[11px] font-extralight text-brand-pink block mb-1">
                        STEP {item.step}
                      </span>
                      <h4 className="text-sm font-light text-slate-900">{item.title}</h4>
        ...
        {/* 3. ASSURANCE GUARANTEE (DARK BREAK) */}
