    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-brand-edge-dark bg-gradient-to-r from-brand-carbon via-brand-carbon/90 to-brand-void p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 shadow-xl">
        ...
        <div className="flex items-center gap-4 bg-brand-void/60 border border-brand-edge-dark p-4 rounded-xl shrink-0">
          <div>
            <span className="text-[10px] font-mono text-brand-mist/50 uppercase block">Performance Index</span>
            <div className="flex items-baseline gap-1 mt-0.5">
              <span className="text-4xl font-light text-emerald-400 tracking-tight">{metrics.performanceRating}</span>
              <span className="text-xs text-brand-mist/40 font-mono">/ 5.0</span>
            </div>
            <span className="text-[10.5px] font-mono text-emerald-400/80 block mt-0.5">Top 5% Network Benchmark</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon/60 p-5 space-y-1">
          <span className="text-[10px] font-mono text-brand-mist/50 uppercase">SLA Attendance</span>
        ...
        <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon/60 p-5 space-y-1">
          <span className="text-[10px] font-mono text-brand-mist/50 uppercase">First-Time Fix</span>
        ...
        <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon/60 p-5 space-y-1">
          <span className="text-[10px] font-mono text-brand-mist/50 uppercase">Evidence Quality</span>
        ...
        <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon/60 p-5 space-y-1">
          <span className="text-[10px] font-mono text-brand-mist/50 uppercase">Recall Rate</span>
        ...
        <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div className="p-3 rounded-lg bg-brand-void border border-brand-edge-dark">
              <span className="text-brand-mist/50 text-[10px] uppercase font-mono block">Acceptance Rate</span>
              <span className="text-xl font-light text-white mt-1 block">{metrics.acceptanceRatePct}%</span>
              <span className="text-[10.5px] text-brand-mist/40 font-mono mt-0.5 block">
                {metrics.totalAssignmentsAccepted} accepted / {metrics.totalAssignmentsOffered} offered
              </span>
            </div>
            <div className="p-3 rounded-lg bg-brand-void border border-brand-edge-dark">
              <span className="text-brand-mist/50 text-[10px] uppercase font-mono block">Avg Response Time</span>
              <span className="text-xl font-light text-cyan-400 mt-1 block">{metrics.avgResponseTimeMinutes} mins</span>
              <span className="text-[10.5px] text-brand-mist/40 font-mono mt-0.5 block">Offer acknowledge speed</span>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon overflow-hidden">
        <div className="p-5 border-b border-brand-edge-dark/60 flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-brand-mist/40">Real Completed Jobs</span>
        </div>
        ...
        {metrics.recentJobs.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-brand-void/80 border-b border-brand-edge-dark text-brand-mist/50 uppercase font-mono text-[10px]">
              ...
              <tbody className="divide-y divide-brand-edge-dark/30">
                {metrics.recentJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-brand-edge-dark/20 transition-colors">
                    <td className="py-3 px-4 font-mono text-white">{job.workOrderNumber}</td>
                    ...
                    <td className="py-3 px-4 font-mono text-white">5.0 / 5.0</td>
                    <td className="py-3 px-4 text-right">
                      <span className="px-2 py-0.5 rounded bg-brand-void text-[10px] font-mono text-brand-mist border border-brand-edge-dark">
                        {job.status}
                      </span>
                    </td>
                  </tr>
