      ...
      <div className="rounded-2xl border border-[#E8E8E5] bg-gradient-to-r from-brand-carbon via-brand-carbon/90 to-brand-void p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 shadow-xl">
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <h1 className="text-2xl sm:text-3xl font-light text-[#111111] tracking-tight">{operative.fullName}</h1>
        ...
        {/* Digital ID Block */}
        <div className="p-4 rounded-xl bg-[#FAFAF8] border border-[#E8E8E5] flex items-center gap-4 shrink-0">
          <QrCode className="w-8 h-8 text-brand-electric" />
          <div className="text-xs font-normal">
            <span className="text-[10px] text-[#9A9A95] block">EntireFM Contractor ID</span>
            <span className="text-[#111111] font-bold block mt-0.5">{operative.entirefmContractorIdNumber}</span>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-[#E8E8E5] bg-white shadow-sm p-6 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <span className="text-[#9A9A95] block">Work Email Address</span>
                <span className="text-[#111111] font-normal mt-0.5 block">{operative.email}</span>
              </div>
              <div>
                <span className="text-[#9A9A95] block">Work Mobile</span>
                <span className="text-[#111111] font-normal mt-0.5 block">{operative.phone || 'Not recorded'}</span>
              </div>
              <div>
                <span className="text-[#9A9A95] block">Home / Base Postcode</span>
                <span className="text-[#111111] font-normal mt-0.5 block">{operative.homePostcode || 'Local Hub'}</span>
              </div>
              <div>
                <span className="text-[#9A9A95] block">Start Date</span>
                <span className="text-[#111111] font-normal mt-0.5 block">{operative.startDate || '—'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-[#E8E8E5] bg-white shadow-sm p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {operative.trades.map((t) => (
                <span
                  key={t}
                  className="px-2.5 py-1 rounded bg-[#FAFAF8] border border-[#E8E8E5] text-[#111111] text-xs font-light"
                >
                  {t}
                </span>
              ))}
              ...
              {operative.qualifications.map((q) => (
                <div key={q.id} className="py-3 flex items-center justify-between gap-3 text-xs">
                  <div className="space-y-0.5">
                    ...
                    <span className="text-[11px] font-normal text-[#9A9A95] block">
            ...
            <div className="divide-y divide-[#E8E8E5]">
              {operative.trainingRecords.map((t) => (
                <div key={t.id} className="py-3 flex items-center justify-between gap-3 text-xs">
                  <div className="space-y-0.5">
                    <span className="text-[#111111] font-normal block">{t.courseName}</span>
                    <span className="text-[11px] font-normal text-[#9A9A95] block">
          ...
          <div className="rounded-xl border border-[#E8E8E5] bg-white shadow-sm p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              {operative.competencies.map((comp) => (
                <div key={comp} className="p-3 rounded-lg bg-[#FAFAF8] border border-[#E8E8E5] flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span className="text-[#111111] font-light">{comp.replace(/_/g, ' ')}</span>
                </div>
              ))}
            </div>
          </div>
