...
import { Mail, RefreshCw, CheckCircle2, ArrowRight, AlertTriangle } from 'lucide-react';
...
function VerifyEmailForm() {
  ...
  const isInvalidOrExpired = errorParam === 'invalid_or_expired';
  const isAlreadyVerified = errorParam === 'already_verified';
    ...
    return (
      <div className="w-full max-w-[480px]">
        <div className="rounded-lg border border-slate-800/80 bg-slate-900/90 p-8 shadow-2xl backdrop-blur-xl text-center space-y-6">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/10 border border-emerald-500/30 mx-auto">
            <CheckCircle2 className="h-8 w-8 text-emerald-400" />
          </div>

          <div className="space-y-2">
            <h1 className="text-2xl font-extralight tracking-tight text-white">Email already verified</h1>
            <p className="text-[13.5px] text-slate-300 leading-relaxed">
              Your email address has already been confirmed. You can sign in to continue your supplier application.
            </p>
          </div>
  ...
  // State B: Invalid or Expired Link
  if (isInvalidOrExpired) {
    return (
      <div className="w-full max-w-[480px]">
        <div className="rounded-lg border border-slate-800/80 bg-slate-900/90 p-8 shadow-2xl backdrop-blur-xl text-center space-y-6">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-amber-500/10 border border-amber-500/30 mx-auto">
            <AlertTriangle className="h-8 w-8 text-amber-400" />
          </div>

          <div className="space-y-2">
            <h1 className="text-2xl font-extralight tracking-tight text-white">We couldn’t verify this email link</h1>
            <p className="text-[13px] text-slate-300 leading-relaxed">
              The verification link may have expired or already been used. Please request a fresh confirmation link or sign in.
            </p>
          </div>

          {/* Email input for resend if not already set */}
          {!email && (
            <div className="text-left space-y-1.5 pt-2">
              <label htmlFor="resend-email" className="block font-light text-[11.5px] uppercase tracking-wider text-slate-400">
                Work Email Address
              </label>
              <input
                id="resend-email"
          ...
          {resent && (
            <div className="rounded border border-emerald-500/30 bg-emerald-500/10 p-3 text-[12.5px] text-emerald-300 flex items-center justify-center gap-2">
              ...
              <span>Fresh verification email sent. Please check your inbox.</span>
            </div>
          )}
          ...
          <div className="border-t border-slate-700/60 pt-5 space-y-3">
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded bg-brand-pink py-2.5 text-[13.5px] font-medium text-white shadow-md transition-all hover:bg-brand-pink/90 disabled:opacity-50"
            >
            ...
            <div className="pt-1">
              <Link
                href="/supplier-portal/sign-in"
                className="inline-flex w-full items-center justify-center gap-2 rounded border border-slate-700 bg-slate-800 py-2.5 text-[13px] font-medium text-slate-200 hover:bg-slate-700 transition-colors"
              >
                Return to Sign In
              </Link>
            </div>
  ...
  // State C: Default Holding / Awaiting Verification Screen
  return (
    <div className="w-full max-w-[480px]">
      <div className="rounded-lg border border-slate-800/80 bg-slate-900/90 p-8 shadow-2xl backdrop-blur-xl text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-extralight tracking-tight text-white">Verify your work email</h1>
          <p className="text-[13.5px] text-slate-300">
            A verification link has been sent to:
          </p>
          {email ? (
            <p className="text-[14px] font-normal text-white bg-slate-800/80 rounded px-4 py-2 break-all">
              {email}
            </p>
          ) : (
