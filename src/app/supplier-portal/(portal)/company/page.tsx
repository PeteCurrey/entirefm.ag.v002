      ...
      <div className="bg-white border border-slate-200 rounded-sm p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-sans">
          <div className="space-y-1">
            <span className="font-bold text-slate-400 text-[10px] uppercase font-mono block">Trading Address</span>
            <p className="text-slate-800">{registeredAddress}</p>
          </div>
          <div className="space-y-1">
            <span className="font-bold text-slate-400 text-[10px] uppercase font-mono block">Direct Field Operatives</span>
          ...
          {draft?.gasSafeNumber || draft?.fGasNumber ? (
            <div className="space-y-2 md:col-span-2 pt-4 border-t border-slate-100">
              <span className="font-bold text-slate-400 text-[10px] uppercase font-mono block">Declared Accreditations</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                {draft.gasSafeNumber && (
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded">
                    <span className="font-bold text-slate-900 block text-xs">Gas Safe Register</span>
                    <span className="text-[11px] font-mono text-emerald-700 font-bold block mt-0.5">Reg: {draft.gasSafeNumber}</span>
                ...
                {draft.fGasNumber && (
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded">
                    ...
                    <span className="text-[11px] font-mono text-emerald-700 font-bold block mt-0.5">Cert: {draft.fGasNumber}</span>
