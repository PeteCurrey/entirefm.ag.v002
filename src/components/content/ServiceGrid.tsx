...
function GridHeading({
  ...
  return (
    <div className="mb-8 sm:mb-12 flex flex-col gap-4 sm:gap-6 md:flex-row md:items-end md:justify-between" data-reveal>
      <div className="max-w-2xl">
        <p className="eyebrow">{eyebrow}</p>
        <h2 className="mt-3 sm:mt-5 text-display-md text-brand-graphite">{title}</h2>
        <p className="prose-brand mt-3 sm:mt-4 text-sm sm:text-base">{intro}</p>
      </div>
          ...
          {SERVICES.map((service, i) => {
            ...
            return (
              ...
              >
                <Link
                  href={service.path}
                  className="on-dark group relative isolate flex h-full flex-col bg-brand-graphite p-5 sm:p-7"
                >
                  ...
                  <h3 className="text-[1rem] sm:text-[1.0625rem] font-light leading-snug tracking-tight text-white">
                    {service.title}
                  </h3>
                  <p className="mt-2.5 sm:mt-3 flex-1 text-xs sm:text-[13.5px] leading-relaxed text-brand-mist/70">
                    {service.desc}
                  </p>

                  {/* Detail is always present; only its presentation is deferred. */}
                  <div className="reveal-on-hover">
                    <ul className="space-y-1.5 pt-3 sm:pt-4">
                      {service.features.map((f) => (
                        <li key={f} className="flex items-start gap-2 text-xs sm:text-[12.5px] text-brand-mist/70">
                          <span
                            aria-hidden="true"
                            className="mt-[6px] sm:mt-[7px] h-1 w-1 shrink-0 rounded-full bg-brand-electric-bright"
                          />
                          {f}
                        </li>
                      ))}
                  ...
                  <span className="mt-5 sm:mt-6 inline-flex items-center gap-1.5 border-t border-white/12 pt-3.5 sm:pt-4 text-xs sm:text-[12.5px] font-normal text-white transition-colors duration-300 group-hover:text-brand-electric-bright">
...
const SECTORS: SectorItem[] = [
  // 4 Primary Visual Anchors
  {
    ...
    category: 'Industrial',
    path: '/manufacturing-industrial-fm',
    imageKey: 'manufacturing-facility',
    desc: 'Heavy mechanical systems, high-voltage distribution, compressed air and continuous plant uptime for high-output sites.',
    isPrimary: true,
  },
  {
    title: 'Corporate Offices & Workplaces',
    category: 'Commercial',
    path: '/commercial-office-fm',
    imageKey: 'city-tower',
    desc: 'Statutory compliance, HVAC efficiency, access control and workplace engineering for single and multi-tenant headquarters.',
    isPrimary: true,
  },
  {
    title: 'Retail Parks & Shopping Centres',
    category: 'Retail & Mixed-Use',
    path: '/retail-parks-shopping-centres-fm',
    imageKey: 'retail-storefront',
    desc: 'High-footfall asset reliability, life safety systems, tenant interface and out-of-hours heavy engineering delivery.',
    isPrimary: true,
  },
  {
    title: 'Logistics, Fulfilment & Distribution',
    category: 'Supply Chain',
    path: '/logistics-distribution-fm',
    imageKey: 'logistics-warehouse',
    desc: 'Dock levellers, high-bay lighting, rapid-roll doors, sprinkler networks and 24/7 reactive cover for critical supply chain nodes.',
    isPrimary: true,
  },

  // 11 Secondary Specialist Sectors
  {
    title: 'Healthcare, Clinical & Care Facilities',
    category: 'Healthcare',
    path: '/healthcare-clinical-fm',
    imageKey: 'clinical-plant',
    desc: 'HTM-compliant water hygiene, backup power generation, medical gas infrastructure and stringent environmental controls.',
  },
  {
    title: 'Education, Universities & Schools',
    category: 'Education',
    path: '/education-facilities-management',
    imageKey: 'university-campus',
    desc: 'Safeguarded DBS-checked engineering teams, term-time maintenance schedules and holiday major-works project windows.',
  },
  {
    title: 'Life Sciences, Biotech & Laboratories',
    category: 'Life Sciences',
    path: '/life-sciences-laboratories-fm',
    imageKey: 'laboratory-cleanroom',
    desc: 'Cleanroom air handling, validated temperature mapping, fume extraction and uninterrupted power systems for sensitive research.',
  },
  {
    title: 'Data Centres & Mission-Critical Infrastructure',
    category: 'Technology',
    path: '/data-centre-facilities-management',
    imageKey: 'data-centre-hall',
    desc: 'N+1 redundancy verification, precision cooling, continuous UPS load testing and 15-minute emergency engineering response.',
  },
  {
    title: 'Automotive Showrooms & Dealerships',
    category: 'Automotive',
    path: '/automotive-dealership-fm',
    imageKey: 'totem-headquarters',
    desc: 'Architectural lighting, workshop extraction, compressor lines, customer hospitality areas, and EV charging station upkeep.',
  },
  {
    title: 'Transport Hubs & Rail Infrastructure',
    category: 'Infrastructure',
    path: '/transport-hubs-rail-fm',
    imageKey: 'train-station',
    desc: 'Public-concourse MEP systems, platform lighting, escalators, life safety auditing, and strict possession-window project engineering.',
  },
  {
    title: 'Local Authority & Public Sector Estates',
    category: 'Public Sector',
    path: '/public-sector-facilities-management',
    imageKey: 'civic-centre',
    desc: 'Framework-compliant asset management, net-zero carbon transition planning, social value delivery, and public building statutory compliance.',
  },
  {
    title: 'Construction Sites & Major Developments',
    category: 'Construction & Capital Works',
    path: '/construction-site-fm',
    imageKey: 'crane-sunset',
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
          {primarySectors.map((sector, i) => (
            <Link
              ...
              className="on-dark group relative isolate flex min-h-[14rem] sm:min-h-[18rem] flex-col justify-between overflow-hidden rounded-sm border border-brand-edge-dark bg-brand-graphite p-5 sm:p-7 transition-all duration-300 hover:border-brand-pink/50"
            >
              ...
              <div>
                ...
                <h3 className="mt-2 text-lg sm:text-xl font-light tracking-tight text-white group-hover:text-brand-pink-light transition-colors">
                  {sector.title}
                </h3>
                <p className="mt-2.5 sm:mt-3 max-w-xl text-xs sm:text-[13.5px] leading-relaxed text-brand-mist/80">
                  {sector.desc}
                </p>
              </div>
              <span className="mt-5 sm:mt-6 inline-flex items-center gap-1.5 border-t border-white/12 pt-3.5 sm:pt-4 text-xs font-normal uppercase tracking-wider text-white transition-colors duration-300 group-hover:text-brand-pink-light">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {secondarySectors.map((sector, i) => (
            <Link
              ...
              className="on-dark group relative isolate flex flex-col justify-between overflow-hidden rounded-sm border border-brand-edge-dark/80 bg-brand-graphite/90 p-4 sm:p-5 backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-brand-carbon"
            >
              <CardBackdrop imageKey={sector.imageKey} />
              <span
                aria-hidden="true"
              ...
              <div>
                ...
                <h4 className="mt-1 sm:mt-1.5 text-sm sm:text-base font-light text-white group-hover:text-brand-electric-bright transition-colors">
                  {sector.title}
                </h4>
                <p className="mt-1.5 sm:mt-2 text-xs leading-relaxed text-brand-mist/70">
                  {sector.desc}
                </p>
              </div>
              <span className="mt-3.5 sm:mt-4 inline-flex items-center gap-1 text-xs sm:text-[11.5px] font-normal text-brand-mist/60 group-hover:text-white transition-colors">
                View scope
      ...
      <div className="container-custom relative section">
        <div className="mb-8 sm:mb-12 flex flex-col gap-4 sm:gap-6 md:flex-row md:items-end md:justify-between" data-reveal>
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 sm:mt-5 text-display-md text-white">
              National coverage, run as regional operations
            </h2>
            <p className="mt-3 sm:mt-4 text-sm sm:text-[15px] leading-relaxed text-brand-mist/60">
          ...
          {cities.map((city, i) => (
            ...
            >
              <Link
                ...
                className="group relative flex h-full flex-col justify-between bg-brand-graphite p-5 sm:p-6 transition-colors duration-500 ease-brand hover:bg-brand-carbon"
              >
                <span
                  aria-hidden="true"
                  className="absolute inset-x-0 top-0 h-px origin-left scale-x-0 bg-brand-spectrum transition-transform duration-500 ease-brand group-hover:scale-x-100"
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] sm:text-[11.5px] uppercase tracking-wider text-brand-mist/40">
                    {city.region}
                  </p>

                  {/* The one thing that makes this city different. */}
                  <div className="reveal-on-hover">
                    <p className="pt-2.5 sm:pt-3 text-[11.5px] sm:text-[12px] leading-relaxed text-brand-mist/60">
                      {city.operatingConditions[0].title}
                    </p>
                  </div>
                </div>
                <span className="mt-4 sm:mt-5 inline-flex items-center gap-1.5 text-xs sm:text-[12px] font-normal text-brand-electric-bright">
