    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon overflow-hidden">
        ...
        <div className="divide-y divide-brand-edge-dark/30">
          {tasks.map((task) => (
            <div key={task.id} className="px-5 py-3 grid grid-cols-12 gap-3 items-center text-xs font-mono">
          ...
          <div className="p-5">
            <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-12 gap-2">
              {plan.months.map((month) => (
                ...
                >
                  <div className="text-[10.5px] font-mono font-bold text-white">{month.monthLabel}</div>
                  {month.scheduledTasks.length === 0 ? (
                    <div className="text-[9px] text-brand-mist/30 font-mono">—</div>
                  ) : (
                    <>
                      <div className="text-[11px] font-mono text-brand-electric-bright font-bold">
                        {month.scheduledTasks.length} visit{month.scheduledTasks.length > 1 ? 's' : ''}
                      </div>
                      <div className="text-[9.5px] text-brand-mist/50 font-mono">{month.totalHours}h</div>
            ...
            {plan.months.map((month) => (
              <div key={month.monthNumber}>
                <button
                  ...
                  className="w-full px-5 py-3 flex items-center justify-between text-xs font-mono hover:bg-brand-edge-dark/10 transition-colors"
                >
                  <div className="flex items-center gap-3">
      ...
      <div className="rounded-xl border border-brand-edge-dark bg-brand-carbon p-5 space-y-3">
        <h3 className="text-xs font-mono text-brand-mist/60 uppercase tracking-widest">Annual Trade Demand Summary</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {Object.entries(disciplineSummary).map(([disc, data]) => (
            <div key={disc} className="p-3 rounded-lg bg-brand-void border border-brand-edge-dark space-y-1">
              <span className="text-[10.5px] font-mono text-brand-mist/60 block">{disc}</span>
              <span className="text-white font-mono font-bold text-sm block">{data.hours}h</span>
