...
export function DocumentUploadModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#FAFAF8] backdrop-blur-sm">
      <div className="bg-white border border-[#E8E8E5] rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-150">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E8E5] bg-[#FAFAF8]">
          <div>
            ...
            <h2 className="text-lg font-light text-[#111111]">Upload Compliance Document</h2>
          </div>
          <button
            onClick={onClose}
            className="text-[#6D6D68] hover:text-white transition-colors p-1 rounded-lg hover:bg-[#F5F5F3]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          {/* File Selector */}
          <div className="border-2 border-dashed border-[#E8E8E5] hover:border-brand-electric/50 transition-colors rounded-xl p-6 text-center cursor-pointer bg-[#FAFAF8] relative">
            <input
              type="file"
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              {selectedFile ? (
                <div>
                  <p className="text-sm font-normal text-[#111111]">{selectedFile.name}</p>
                  <p className="text-xs text-[#9A9A95] mt-0.5 font-normal">
                    {(selectedFile.size / 1024).toFixed(1)} KB &bull; Click or drop to replace
                  </p>
                </div>
              ) : (
                <div>
                  <p className="text-sm font-normal text-[#111111]">Drag &amp; drop document or click to browse</p>
                  <p className="text-xs text-[#9A9A95] mt-0.5">PDF, PNG, JPG up to 25MB</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Document Title *</label>
              <input
                type="text"
                required
                value={docTitle}
                ...
                className="w-full px-3.5 py-2.5 rounded-lg bg-[#FAFAF8] border border-[#E8E8E5] text-white text-sm focus:border-brand-electric focus:outline-none"
              />
            </div>

            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Category *</label>
              <select
                value={category}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Insurer / Awarding Body</label>
              <input
                type="text"
                value={insurer}
            ...
            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Policy / Reg Number</label>
              <input
                type="text"
                value={policyNumber}
            ...
            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Cover Limit (£)</label>
              <input
                type="number"
                value={coverLimit}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Issue Date</label>
              <input
                type="date"
                value={issueDate}
            ...
            <div>
              <label className="text-xs font-normal text-[#111111] block mb-1.5">Expiry Date (if applicable)</label>
              <input
                type="date"
                value={expiryDate}
          ...
          <div>
            <label className="text-xs font-normal text-[#111111] block mb-1.5">Notes &amp; Scope Summary</label>
            <textarea
              rows={2}
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Add optional notes, cover endorsements, or certificate scope..."
              className="w-full px-3.5 py-2 rounded-lg bg-[#FAFAF8] border border-[#E8E8E5] text-white text-sm focus:border-brand-electric focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#E8E8E5]">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-lg border border-[#E8E8E5] text-sm font-normal text-[#111111] hover:text-white hover:bg-[#FAFAF8] transition-colors"
            >
              Cancel
            </button>
            <button
