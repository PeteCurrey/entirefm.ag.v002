        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {LOBBY_CORE_DESTINATIONS.map((card) => (
            <Link
              key={card.id}
              href={card.href}
              className="group relative flex flex-col justify-between overflow-hidden rounded-[4px] min-h-[380px] sm:min-h-[420px] lg:min-h-[440px] p-7 sm:p-8 text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-electric shadow-xs"
            >
              ...
              <div
                aria-hidden="true"
                className="absolute inset-0 -z-10 bg-gradient-to-t from-black/92 via-black/60 to-black/30 transition-opacity duration-500 group-hover:from-black/96 group-hover:via-black/70 group-hover:to-black/40"
              />

              {/* Subtle hairline edge */}
              <div
                aria-hidden="true"
                className="absolute inset-0 -z-10 border border-white/10 group-hover:border-white/20 transition-colors pointer-events-none rounded-[4px]"
              />

              {/* Top Meta: Number + Category Tag */}
              <div className="flex items-start justify-between gap-4">
                <span className="font-mono text-xs tracking-widest text-white/50 group-hover:text-white/80 transition-colors">
                  {card.number}
                </span>
                <span className="text-[9px] font-medium uppercase tracking-[0.2em] text-white/60 bg-white/10 backdrop-blur-xs px-2.5 py-1 rounded-[2px] border border-white/10">
                  {card.categoryTag}
                </span>
              </div>
              ...
              <div className="space-y-3 pt-12">
                ...
                <div>
                  ...
                  <p className="text-sm sm:text-base font-light text-white/90 mt-1.5 leading-snug">
                    {card.supportingLine}
                  </p>
                </div>

                {/* Supporting Description — slides/fades into view on hover */}
                <p className="text-xs font-light text-white/75 leading-relaxed line-clamp-3 transition-all duration-400 ease-out md:max-h-0 md:opacity-0 md:group-hover:max-h-24 md:group-hover:opacity-100">
                  {card.description}
                </p>

                {/* Primary Action Button / Indicator */}
                <div className="pt-2 flex items-center justify-between border-t border-white/15 text-xs font-normal tracking-wider uppercase text-white/90 group-hover:text-white transition-colors">
