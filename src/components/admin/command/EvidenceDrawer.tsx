...
export function EvidenceDrawer({ evidence, toolRuns, computedAt }: Props) {
  ...
  return (
    <div className="mt-3 border-t border-[#E8E8E5] pt-3">
      <button
        onClick={() => setOpen(v => !v)}
        className="flex items-center gap-1.5 text-[11px] font-mono text-[#6D6D68] hover:text-[#111111] transition-colors"
      >
        ...
        <div className="mt-3 space-y-3">
          {evidence.length > 0 && (
            <div className="rounded-[6px] border border-[#E8E8E5] bg-[#FAFAF8] p-3.5 space-y-2">
              <div className="text-[10px] font-mono text-[#6D6D68] uppercase tracking-wider">Evidence Metrics Ledger</div>
                ...
                {evidence.map((ev, i) => (
                  <div key={i} className="flex items-start justify-between gap-4 text-[12px] pt-2 first:pt-0">
                    <div className="text-[#111111] flex-1 min-w-0">
                      ...
                      {ev.source_service && (
                        <div className="text-[10px] font-mono text-[#6D6D68] mt-0.5">{ev.source_service}</div>
                      )}
                      {ev.period && (
                        <div className="text-[10px] font-mono text-[#9A9A95]">{ev.period.label}: {ev.period.from} → {ev.period.to}</div>
                      )}
                      {ev.coverage_pct !== undefined && (
                        <div className={`text-[10px] font-mono ${ev.coverage_pct >= 80 ? 'text-[#15803D]' : 'text-[#B45309]'}`}>
                          Data Coverage: {ev.coverage_pct}%
                        </div>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-[#111111] font-mono font-medium">{typeof ev.value === 'number' ? ev.value.toLocaleString('en-GB') : String(ev.value ?? '—')}</div>
                      <div className={`text-[9.5px] font-mono px-1 py-0.2 rounded border inline-block mt-0.5 ${DATA_STATUS_STYLE[ev.data_status] || 'text-[#6D6D68]'}`}>
                        {ev.data_status}
                      </div>
                    </div>
                  </div>
          ...
          {toolRuns && toolRuns.length > 0 && (
            <div className="rounded-[6px] border border-[#E8E8E5] bg-[#FAFAF8] p-3.5 space-y-2">
              <div className="text-[10px] font-mono text-[#6D6D68] uppercase tracking-wider">Telemetry Runs</div>
              <div className="space-y-1.5">
                {toolRuns.map((tr, i) => (
                  <div key={i} className="flex items-center justify-between text-[11.5px] font-mono">
          ...
          {computedAt && (
            <div className="text-[10px] font-mono text-[#9A9A95] text-right">
