...
export function AdminSourceHealthClient() {
  ...
  const getStatusBadge = (status: IntelligenceSource['healthStatus']) => {
    switch (status) {
      case 'LIVE':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-normal text-emerald-400 bg-emerald-500/10 px-2 py-0.5 border border-emerald-500/20">
      ...
      case 'DEGRADED':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-normal text-amber-400 bg-amber-500/10 px-2 py-0.5 border border-amber-500/20">
      ...
      case 'FAILED':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-normal text-rose-400 bg-rose-500/10 px-2 py-0.5 border border-rose-500/20">
      ...
      case 'CREDENTIAL_REQUIRED':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-normal text-purple-400 bg-purple-500/10 px-2 py-0.5 border border-purple-500/20">
            <Key className="w-3 h-3" />
            KEY REQUIRED
          </span>
        );
      default:
        return <span className="text-[10px] font-normal text-white/40">DISABLED</span>;
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white/[0.03] border border-white/10 p-5 rounded-sm">
          <div className="text-[10px] font-normal uppercase text-white/40">Canonical Items</div>
          <div className="text-2xl font-light text-white mt-1">{counts.totalItems}</div>
        </div>
        <div className="bg-white/[0.03] border border-white/10 p-5 rounded-sm">
          <div className="text-[10px] font-normal uppercase text-white/40">Statutory / Legal</div>
          <div className="text-2xl font-light text-emerald-400 mt-1">{counts.statutoryItems}</div>
        </div>
        <div className="bg-white/[0.03] border border-white/10 p-5 rounded-sm">
          <div className="text-[10px] font-normal uppercase text-white/40">Pending Review</div>
          <div className="text-2xl font-light text-amber-400 mt-1">{counts.pendingReview}</div>
        </div>
        <div className="bg-white/[0.03] border border-white/10 p-5 rounded-sm">
          <div className="text-[10px] font-normal uppercase text-white/40">Total Ingestion Runs</div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={syncing}
          className="inline-flex items-center gap-2 bg-brand-electric text-white text-xs font-medium uppercase tracking-wider px-4 py-2 rounded-sm hover:brightness-110 disabled:opacity-50"
        >
      ...
      <div>
        ...
        <div className="space-y-2">
          {runs.map((run) => (
            <div
              key={run.id}
              className="bg-white/[0.02] border border-white/5 p-3 rounded-sm flex items-center justify-between text-xs font-normal"
            >
