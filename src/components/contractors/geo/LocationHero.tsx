...
export function LocationHero({
  ...
  imageAlt,
  breadcrumbs,
  primaryCta = { label: "Apply to Join EntireFM", href: "/suppliers/apply" },
  secondaryCta = { label: "Explore Contractor Portal", href: "/suppliers/membership#platform-overview" },
  statusBadge = "Active Regional Network",
}: LocationHeroProps) {
  return (
      ...
      <div className="container-wide space-y-8">
        ...
        <div className="max-w-3xl space-y-6">
          <div className="inline-flex items-center gap-2 rounded-sm border border-white/10 bg-white/[0.04] px-3 py-1 text-[11px] font-mono tracking-wider text-brand-mist/90 backdrop-blur-sm">
            ...
            {eyebrow} // {locationName.toUpperCase()} &bull; {region.toUpperCase()}
          </div>
          ...
          <dl className="pt-4 grid grid-cols-2 sm:grid-cols-3 gap-px bg-white/10 border border-white/10 rounded-sm overflow-hidden max-w-xl">
            <div className="bg-slate-900/80 px-4 py-3 backdrop-blur-sm">
              <dt className="text-sm font-semibold text-white">{locationName}</dt>
            ...
            <div className="bg-slate-900/80 px-4 py-3 backdrop-blur-sm col-span-2 sm:col-span-1">
              ...
              <dd className="text-[11px] text-slate-400 font-light mt-0.5">Vetted Allocation</dd>
            </div>
          </dl>
        </div>
      </div>
