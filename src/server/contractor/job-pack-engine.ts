...
export async function assembleJobPack(
  ...
  const pack: JobPackRecord = {
    ...
    contractorName: session.orgName || 'Contractor',
    clientName: client.name || 'Client',
    siteId: site.id,
    siteName: site.name || 'Site',
    siteAddress: [site.address_line1, site.city, site.postcode].filter(Boolean).join(', ') || 'Address on file',
    trade: workOrder.trade || 'GENERAL_MAINTENANCE',
    ...
    ramsRecord,
    siteInstructions: {
      accessHours: site.access_hours || 'Standard Access',
      contactName: site.contact_name || 'Site Management',
      contactPhone: site.contact_phone || '',
      parkingInstructions: site.parking_instructions || 'Refer to site access protocols.',
      securityInductionRequired: Boolean(site.security_induction_required ?? true),
      knownAsbestosLocation: site.asbestos_notes || 'Refer to site Asbestos Register before intrusive work.',
