...
// Helper to seed initial sample conversation for demo member
function seedInitialDMs() {
  if (CONVERSATIONS_STORE.size > 0) return;

  const convId = 'conv-seed-01';
  ...
  const conversation: Conversation = {
    ...
    participants: [
      ...
      {
        memberId: 'mem-seed-eng-01',
        memberName: 'Marcus Vance',
        memberHeadline: 'Senior Mechanical Engineer | CEng MCIBSE',
        memberCompany: 'Vance Engineering Associates',
        status: 'accepted',
        joinedAt: now,
        lastReadAt: now,
        muted: false,
  ...
  const messages: DirectMessage[] = [
    ...
    },
    {
      ...
      body: 'Thanks Pete, we used the acoustic tension meter on AHU-2 today and Hz readings were spot on. Turns out the driven pulley sheave was stepped by 1.2mm.',
      createdAt: new Date(Date.now() - 1000 * 60 * 15).toISOString(),
      moderationState: 'published',
    },
  ];

  DIRECT_MESSAGES_STORE.set(convId, messages);
}
