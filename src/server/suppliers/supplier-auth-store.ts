...
export function mapDbDraftToRecord(row: any): SupplierApplicationDraft {
  return {
    orgId: row.org_id,
    applicationReference: row.application_reference,
...
function mapDraftRecordToDb(draft: SupplierApplicationDraft): Record<string, any> {
  return {
    ...
    has_247: Boolean(draft.has247),
    emergency_sla_hours: draft.emergencySlaHours,
    has_subcontractors: Boolean(draft.hasSubcontractors),
    ...
    has_hs_policy: Boolean(draft.hasHsPolicy),
    has_rams: Boolean(draft.hasRams),
    has_incident_history: Boolean(draft.hasIncidentHistory),
    anti_bribery: Boolean(draft.antiBribery),
    modern_slavery: Boolean(draft.modernSlavery),
