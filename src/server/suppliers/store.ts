...
function mapDbOrgToSupplierRecord(org: any, prov?: any): SupplierOrganisationRecord {
  ...
  return {
    ...
    risk_level: 'LOW',
    headquarters_city: addr.city || '',
    headquarters_postcode: addr.postcode || '',
    full_address: addr.line1 ? `${addr.line1}, ${addr.city || ''} ${addr.postcode || ''}`.trim() : (addr.city || 'United Kingdom'),
    phone: org.phone || '',
    email: org.email || '',
    website_url: org.website || undefined,
    is_national: Boolean(org.settings?.is_national ?? prov?.is_national ?? false),
    emergency_24_7: Boolean(org.settings?.emergency_24_7 ?? prov?.emergency_24_7_capable ?? false),
    services: prov?.primary_trade
      ...
      : [],
    coverage: Boolean(org.settings?.is_national ?? prov?.is_national)
      ? [{
          id: `cov-${org.id}`,
