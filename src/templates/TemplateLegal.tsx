          ...
          <div className="container-custom max-w-4xl space-y-8">
            ...
            <div className="rounded-2xl border border-indigo-200 bg-indigo-50/70 p-6 sm:p-8">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Link
                  href={target.path}
                  className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-bold text-white shadow-xs transition-colors hover:bg-slate-800 shrink-0"
                >
                  View {target.name}
            ...
            <div className="prose prose-slate max-w-none text-slate-700 space-y-6 text-sm sm:text-base leading-relaxed">
              <h2 className="text-xl font-bold text-brand-graphite">1. Corporate Policy Overview & Commitment</h2>
              <p>
              ...
              <h2 className="text-xl font-bold text-brand-graphite">2. Statutory Framework & Compliance</h2>
              <p>
              ...
              {content.sections && content.sections.map((sec, idx) => (
                <div key={idx} className="pt-4 space-y-3">
                  <h2 className="text-xl font-bold text-brand-graphite">{sec.heading}</h2>
                  <p>{sec.body}</p>
                </div>
              ))}

              <h2 className="text-xl font-bold text-brand-graphite">3. Central Governance Centre</h2>
              <p>
                Explore our full suite of 24 corporate, privacy, AI transparency, client, and contractor policies in the{' '}
                <Link href="/legal" className="text-indigo-600 font-semibold hover:underline">
                  EntireFM Legal, Data & Governance Centre
                </Link>.
              </p>

              <h2 className="text-xl font-bold text-brand-graphite">4. Contact & Inquiries</h2>
              <p>
                For questions regarding this policy or our corporate compliance standards, please contact our compliance desk at{' '}
                <a href={`mailto:${CONTACT_CONFIG.enquiryEmail}`} className="text-brand-graphite font-semibold hover:underline">
                  {CONTACT_CONFIG.enquiryEmail}
                </a>{' '}
                or call{' '}
                <a href={CONTACT_CONFIG.mainPhone.href} className="text-brand-graphite font-semibold hover:underline">
